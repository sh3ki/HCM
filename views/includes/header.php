    ...
    <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
        ...
        <div class="space-y-3">
            <div>
                ...
                <input id="new-password" type="password" class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Enter new password" />
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Confirm Password</label>
                <input id="confirm-password" type="password" class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Confirm new password" />
            </div>
            <div class="flex gap-2">
                <button id="password-change-confirm" class="flex-1 bg-primary text-white px-4 py-2 rounded-lg hover:bg-blue-700">Change Password</button>
                <button id="password-change-skip" class="flex-1 bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300">Skip for Now</button>
            </div>
            <p id="password-change-message" class="text-sm text-red-600"></p>
        </div>
    </div>
</div>
<?php endif; ?>

<!-- OTP Verification Modal (appears after password change for new users) -->
<?php if (!empty($_SESSION['is_new'])): ?>
<div id="otp-modal" class="fixed inset-0 z-50 flex items-center justify-center" style="display: none;">
    <div class="fixed inset-0 bg-gray-900 bg-opacity-70"></div>
    <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
